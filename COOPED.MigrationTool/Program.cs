...
foreach (var fichier in fichiers)
{
    ...
    try
    {
        ...
        foreach (var groupeClient in groupesClients)
        {
            string nom = groupeClient.Key.Nom ?? "";
            string prenoms = groupeClient.Key.Prenoms ?? "";
            if (string.IsNullOrWhiteSpace(nom) && string.IsNullOrWhiteSpace(prenoms)) { ignorees += groupeClient.Count(); continue; }
            ...
            foreach (var groupeTontine in groupesTontines)
            {
                try
                {
                    ...
                    var lignes = groupeTontine.OrderBy(e => e.Date is null).ThenBy(e => e.Date).ToList();
                    ...
                    // Date de repli utilisée quand une ligne n'a pas de date : la dernière date connue
                    // rencontrée jusque-là, sinon la date de création de la tontine.
                    DateOnly derniereDateConnue = tontine.DateCreation;

                    for (int i = 0; i < lignes.Count; i++)
                    {
                        var enr = lignes[i];

                        // On ne saute plus une ligne à cause d'une case vide : Date/Nombre/Montant
                        // manquants sont remplacés par une valeur par défaut plutôt que d'ignorer la ligne.
                        DateOnly dateCotisation = enr.Date ?? derniereDateConnue;
                        derniereDateConnue = dateCotisation;

                        int nombre = enr.Nombre ?? 0;
                        ...
                        context.Cotisations.Add(new Cotisation
                        {
                            ...
                            TontineId = tontine.Numero,
                            Date = dateCotisation,
                            Montant = enr.Montant ?? (mise * nombre),
                            NbreMise = nombre,
                            Position = positions[i]
                        });
                        cotisationsCreees++;
...
static TypeTontine MapperType(string? typeBrut, string nomFichier, List<string> rapportTypesInconnus)
{
    ...
    if (t.Equals("Quinzaine", StringComparison.OrdinalIgnoreCase) ||
        t.Equals("Mensuel", StringComparison.OrdinalIgnoreCase) ||
        t.Contains("quinzaine", StringComparison.OrdinalIgnoreCase) ||
        t.Contains("mensuel", StringComparison.OrdinalIgnoreCase))
        return TypeTontine.Pret;
...
static List<Enregistrement> LireEnregistrements(string chemin)
{
    ...
    foreach (var ligne in feuille.RowsUsed().Skip(1))
    {
        // On ne saute une ligne dès la lecture que si elle n'a ni nom ni prénom de client
        // (les autres cases vides, elles, sont conservées et remplies avec des valeurs par défaut plus loin).
        bool ligneSansClient =
            string.IsNullOrWhiteSpace(ligne.Cell(4).GetString()) &&
            string.IsNullOrWhiteSpace(ligne.Cell(5).GetString());

        if (ligneSansClient) continue;
